...
export default function HomeTutor() {
  ...
  function filterbyserach(){
    const temproom=duplicaterooms.filter(rooms=>rooms.course.toLowerCase().includes(searchkey.toLowerCase()))
    setrooms(temproom)
    }
  function filterbylocation(){
    const temp=duplicate.filter(rooms=>rooms.address.toLowerCase().includes(searchke.toLowerCase()))
    setrooms(temp)
    }
  return (
   <>
   ...
   <div class="relative mb-4">
    ...
    {/* <div class="flex items-center">
        ...
        <input type="text"className="form-control bs"placeholder="Search by location"  class="w-full bg-white rounded-l-md border italic border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" value={searchke} onChange={(e)=>{setsearchke(e.target.value)}} onKeyUp={filterbylocation}/>
